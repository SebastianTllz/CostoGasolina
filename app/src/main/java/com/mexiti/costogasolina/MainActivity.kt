...
fun CostGasLayout(name: String) {
    ...
    val montoBase = precioLitro *cantLitros

    val propina = propinaEntrada.toDoubleOrNull() ?: 0.0

    val totalDouble = if (incluirPropina) montoBase + propina else montoBase
    val total = NumberFormat.getCurrencyInstance().format(totalDouble)
    ...
    )
    {
        ...
        EditNumberField(
            ...
            leadingIcon = R.drawable.propina, // Se mantiene como recurso drawable
            keyboardsOptions = KeyboardOptions.Default.copy(
                keyboardType = KeyboardType.Number,
